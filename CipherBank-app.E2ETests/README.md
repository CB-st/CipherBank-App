...
Story IDs and ownership: [`docs/tests/STORY_ID_MAP.md`](../docs/tests/STORY_ID_MAP.md). Coding standards: repo-root [`AGENTS.md`](../AGENTS.md).
...
E2E_RUN=1 TEST_PLATFORM=android \
  ...
  --filter "FullyQualifiedName~CB_ACCOUNT|FullyQualifiedName~US_ONB_03|FullyQualifiedName~US_ONB_04"

# Single story by method-name substring (hyphens → underscores in filter)
E2E_RUN=1 TEST_PLATFORM=android ANDROID_APK_PATH="$APK" \
  dotnet test CipherBank-app.E2ETests/CipherBank-app.E2ETests.csproj \
  --filter "FullyQualifiedName~CB_ACCOUNT_001"
```
...
Default journaled PIN: **`246810`** (`E2E_TEST_PIN`). Alternate PIN for change-PIN: `E2E_TEST_PIN_ALT` (default `135791`).  
Recovery password for CB-ACCOUNT-002: `E2E_RECOVERY_PASSWORD` (default `Cb-Emu-Recovery-2026`).
...
| Path | Contents |
|------|----------|
| `artifacts/e2e-journal/` | Per-story journal (PIN, mnemonic, steps) — **gitignored** |
...
- [`docs/superpowers/specs/2026-07-25-maui-appium-story-completion-design.md`](../docs/superpowers/specs/2026-07-25-maui-appium-story-completion-design.md) — design  
