...
public sealed class AndroidNdefPresentmentService : INfcPresentmentService
{
    ...
    public async Task<bool> PresentAsync(NfcPresentmentPayload payload, TimeSpan? timeout = null, CancellationToken ct = default)
    {
        LastError = null;
        var activity = Platform.CurrentActivity;
        if (activity is null)
        ...
        var flags = NfcReaderFlags.NfcA | NfcReaderFlags.NfcB | NfcReaderFlags.NfcF | NfcReaderFlags.NfcV | NfcReaderFlags.SkipNdefCheck;
        adapter.EnableReaderMode(activity, callback, flags, null);

        TimeSpan wait = timeout ?? TimeSpan.FromSeconds(30);
        using var linked = CancellationTokenSource.CreateLinkedTokenSource(ct);
        linked.CancelAfter(wait);
