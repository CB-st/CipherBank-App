...
## Sonar quality-gate pass (PR #25 follow-up)

Questionnaire-resolved tradeoffs against earlier review notes:

- **IDE0008 / `var`:** Gate treats INFO as failure (`new_violations > 0`). Explicit types were applied
  stack-wide even though the earlier review preferred `var`.
- **S2360 / optional parameters:** ChartMath and CancellationToken APIs use overloads again so Sonar
  CRITICAL S2360 clears, overriding the earlier request to collapse overloads into optional parameters.
