...
internal static class LocalDbSql
{
    ...
    public static async Task ApplyCompatibilityAsync(DbConnection connection, CancellationToken ct)
    {
        ...
        foreach (var upgrade in RecipientColumns)
        {
        ...
        foreach (var scrub in RecipientScrubs)
        {
    ...
    [SuppressMessage(
        ...
        Justification = "CommandText is assigned only from CompatibilityStatement switch arms that return compile-time SQL literals; no user input reaches the query.")]
