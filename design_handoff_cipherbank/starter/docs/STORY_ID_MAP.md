...
**Source of truth for procedures:** scaffold `docs/USER_STORIES.md` + `src/stories/catalog.ts`.  
**This file:** mapping + Expo readiness only.

| Scaffold `CB-*` | Expo `US-*` | Expo surface | Status |
|-----------------|-------------|--------------|--------|
| CB-ACCOUNT-001 | US-ONB-01 | Welcome → Keys → Quiz → PIN → Home | Executable (`e2e/stories/onboarding.spec.ts`) |
| CB-ACCOUNT-002 | US-ONB-02 | Welcome → Set up this device → Keys…PIN | Backlog (`test.fixme`) |
...
| CB-FUND-001 | US-RCV-01 + fund | Receive / deposit into user wallet | Backlog |
...
| CB-PAY-003 | US-POS-01 | POS / prepaid presentment | Backlog (map to Tap to pay lab) |
| CB-MARKET-001 | US-HOM-05 / US-CNV-01 | Home chart + Convert iquote | Backlog |
...
| ID | Spec |
|----|------|
| US-ONB-04 | PIN mismatch — `onboarding.spec.ts` |
